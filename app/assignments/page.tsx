...
export default function AssignmentsPage() {
  ...
  const reassignMeal = (id: string, memberId: string | null) =>
    update((d) => {
  ...
  const unassignedMeals = state.meals.filter((m) => !m.assignedMemberId);
  ...
  return (
    <div className="space-y-5">
      ...
      {state.members.map((m) => {
        ...
        const myMeals = state.meals.filter((x) => x.assignedMemberId === m.id);
        const total = myItems.length + myGrocery.length + myMeals.length;
        return (
          <Card key={m.id}>
      ...
      <div>
        ...
        <div className="space-y-2">
          {unassignedMeals.map((x) => (
            <Card key={x.id} className="!p-3">
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm">🍽️ {x.name}</span>
                <MemberSelect
                  members={state.members}
                  value={null}
                  onChange={(id) => reassignMeal(x.id, id)}
                />
              </div>
            </Card>
          ))}
